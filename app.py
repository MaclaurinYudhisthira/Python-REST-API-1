...
def getNBooks(n):
    if n>len(Data['books']):
        ...
        return jsonify({'Books':Data['books'][:n]})

@app.route("/post",methods=['POST'])
def postBook():
    # print(request.args) # for ulr/somthing?query=thing
    print(request.headers) # for request headers
    # print(request.data) # data other tha form/url encoded/json 
    # print(request.form) # for form/url encoded data
    # print(request.json) # for guess what
    
    # https://tedboy.github.io/flask/generated/generated/flask.Request.html

    Data['books'].append(dict(request.json))
    return jsonify({'Added':dict(request.json)})

@app.route("/put/<int:n>",methods=['PUT'])
def put(n):
    if n>len(Data['books']) or n<1:
        return "Bad Query"
    else:
        Data['books'][n-1]['description']="Updated by PUT"
        return jsonify({'Updated':Data['books'][n-1]})
...
def delete(n):
    if n>len(Data['books']) or n<1:
        ...
        d=Data['books'].pop(n-1)
        return jsonify({'Deleted':d})
